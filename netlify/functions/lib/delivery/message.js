...
function orderAddress(order) {
  ...
  // diner) use `address` (free text) or `deliveryZone.label` (a named zone); 6bashuk
  // uses `address` or `destination`. Kept identical across all three copies since the
  // shared webhook hub (telegram-webhook.js, in the 6bashuk repo) rebuilds messages
  // for every business and needs to handle every shape.
